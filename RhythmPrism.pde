...
void download() 
{
  ...
  menuWindow.setSong("neon-skies.mp3");
  menuWindow.changeAmplitude(0.1);
  menuWindow.setDrums();
  
  gameWindow.loadFiles();
  gameWindow.setSong("BeatMekanik_OutLoud.mp3");
  gameWindow.setDrums();
...
void mousePressed() 
{
  ...
  {         gameWindow.checkHit(); // Now checks hits in the rhythm game
    ...
    else 
    {
      ...
      menuWindow.optionSelected("Easy song", width/2+3, height/4+3) ||
      menuWindow.optionSelected("Hard song", width/2+3, 3*height/4+3)) 
      {
        ...
        gameWindow.difficulty[1] = menuWindow.optionSelected("Easy song", width/2+3, height/4+3);
  ...
  if (gameWindow.getStatus()) 
  {
    ...
    float distance = dist(mouseX, mouseY, 100, height/2);
    if (distance < 50) {
